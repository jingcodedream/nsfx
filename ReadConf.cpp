    ...
    while (context.find("\n") != string::npos) {
        ...
        if (effecStr[0] == '[') {
            if (effecStr.find("]") == string::npos
                    || effecStr[effecStr.size() - 1] != ']') //如果不完整或者有多余字符都是错的
                            {
            ...
            string attrGroup;
            attrGroup = effecStr.substr(effecStr.find("[") + 1,
                    effecStr.find("]") - effecStr.find("[") - 1); //选出组的名称

            if (attrGroup.empty()
                    || find_if(attrGroup.begin(), attrGroup.end(),
                            std::ptr_fun(::isspace)) != attrGroup.end()) //如果是空的或者含有空格都是错的
                            {
            ...
            //插入组中
            std::pair<GroupMap::iterator, bool> groupMapInsertPair =
                    groupMap.insert(
                            std::make_pair<string, KeyValueMap>(attrGroup,
                                    tempKeyValueMap));
            ...
            if (key.empty() || value.empty()) {
                errMsg = "key='" + key + "' is empty or value='" + value
                        + "' is empty";
                return false;
            }
            if (find_if(key.begin(), key.end(), std::ptr_fun(::isspace))
                    != key.end()
                    || find_if(value.begin(), value.end(),
                            std::ptr_fun(::isspace)) != value.end()) {
                errMsg = "key='" + key + "' has space or value='" + value
                        + "' has space";
                return false;
            }

            std::pair<KeyValueMap::iterator, bool> keyValueMapTempInsertPair =
                    KeyValueMapTemp->insert(
                            std::make_pair<string, string>(key, value));
            if (keyValueMapTempInsertPair.second == false) {
                errMsg = "insert key'" + key + "' value'" + value
                        + "' into keyValueMap failed";
                return false;
            }
        }
        context = context.substr(context.find("\n") + 1);
...
void ReadConf::GetValue(const string &group, const string &key, int &value,
        int default_value) {
...
void ReadConf::GetValue(const string &group, const string &key, string &value,
