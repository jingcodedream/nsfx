...
std::pair<IOEvent, IOEvent> IOServer::AddEvent(int fd, IOEvent event, IOHandle *handle)
{
    ...
    if(it == fdIOEventInfoMap.end())
    {
        ...
        std::pair<std::map<int, IOEventInfo>::iterator, bool> retInsert=fdIOEventInfoMap.insert(std::make_pair(fd, ioEventInfoTemp));
...
std::pair<IOEvent, IOEvent> IOServerEpoll::AddEvent(int fd, IOEvent event, IOHandle *handle)
{
    std::pair<IOEvent, IOEvent> ret_pair= IOServer::AddEvent(fd, event, handle);
...
bool IOServerEpoll::WaitEvent(std::vector<EventOccur> &eventOccurVector, int wait_ms)
{
    eventOccurVector.clear();
    int32_t eventCount = 0;
    if((eventCount = epoll_wait(epollFd, epollEvent, maxEventNum, wait_ms)) == -1)
    {
        if(errno != EINTR)
        {
...
bool IOServer::RunOnce()
{
    ...
    for(int i = 0;i < eventCount;++i)
    {
        ...
        std::map<int, IOEventInfo>::iterator it = fdIOEventInfoMap.find(currentFd);
        ...
        {
            std::pair<IOEvent, IOEvent> ret_pair = DelEvent(eventOccurTemp.fd, delEvents);
